...
import { Cnpj } from '@app/entities/Supplier/Cnpj'
import { Cpf } from '@app/entities/Supplier/Cpf'
...
export class PrismaSuppliersMapper {
  static toPrisma(supplier: Supplier): RawSupplier {
    return {
      ...
      cpf: supplier.cpf?.value ?? null,
      cnpj: supplier.cnpj?.value ?? null,
      ddd: supplier.ddd,
    ...
    return new Supplier(
      {
        ...
        cpf: rawSupplier.cpf ? new Cpf(rawSupplier.cpf) : null,
        cnpj: rawSupplier.cnpj ? new Cnpj(rawSupplier.cnpj) : null,
        ddd: rawSupplier.ddd,
